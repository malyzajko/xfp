...
object RationalForm {
  def apply(interval: Interval): RationalForm = {
...
case class RationalForm(val x0: Rational, var noise: Queue[Deviation]) {
  ...
  val interval: (Rational, Rational) = {
    val rad = radius
    (x0 - rad, x0 + rad)
  }

  def intervalDouble = Interval(interval._1.toDouble, interval._2.toDouble)
